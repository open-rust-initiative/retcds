...
use crate::{BaseStatus, config::Config, INVALID_ID, StateRole};
use crate::{storage::GetEntriesFor, GetEntriesContext, Raft, SoftState, Storage};

use slog::info;
use raft_proto::eraftpb::ConfChangeType;
...
/// The RawNode is a thread-safe representation of a Node.
#[derive(Clone)]
pub struct SafeRawNode<S: Storage> {
    pub(crate) raw_node: Arc<RwLock<RawNode<S>>>,
}

impl<S: Storage> SafeRawNode<S> {
    /// Creates a new RawNode given the configuration and the storage engine.
    pub fn new(raw_node: RawNode<S>) -> Self {
        SafeRawNode {
    ...
    pub fn rl(&self) -> RwLockReadGuard<'_, RawNode<S>> {
        self.raw_node.read().unwrap()
    }


    /// Returns the ID of the current node.
    pub fn wl(&self) -> RwLockWriteGuard<'_, RawNode<S>> {
        self.raw_node.write().unwrap()
    }
}
...
impl Ready {
    /// Creates a new `Ready` instance.
    pub fn new<S: Storage, T: Into<Option<SoftState>>>(
        raft: &Raft<S>,
        prev_soft_st: T,
        prev_hard_st: HardState,
    ) -> Ready {
        let mut prev_soft_st =prev_soft_st.into();
        let mut light = LightReady{
            commit_index: Some(raft.raft_log.committed),
            committed_entries: raft.raft_log.all_entries(),
            messages: raft.msgs.clone()
        };
        let mut ready= Ready{
            number: Default::default(),
            ss: prev_soft_st.clone(),
            hs: Option::from(prev_hard_st.clone()),
            read_states: vec![],
        ...
        if prev_soft_st.is_some() && raft.soft_state()!=prev_soft_st.unwrap() {
            ready.ss = Some(raft.soft_state());
        }
        if prev_hard_st != raft.hard_state() {
            ready.hs = Option::from(raft.hard_state());
        }
...
impl<T: Storage> RawNode<T> {
    ...
    pub(crate) fn accept_ready(&mut self, ready: &Ready) {
        ...
        if ready.ss.is_some() {
            self.prev_ss = ready.ss.clone().unwrap();
        }
    ...
    /// Set whether to enable the pre-vote algorithm at runtime.
    pub fn bootstrap(&mut self,peers: Vec<Peer>) -> Result<()> {
        if peers.is_empty() {
        ...
        self.prev_hs = HardState::new();

        self.raft.become_follower(1, INVALID_ID);
